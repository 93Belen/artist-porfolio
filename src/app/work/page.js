...
if(idsUrls.length === 0){
    ...
    return (
        <div className='grid min-h-screen gap-9 md:grid-cols-3 md:grid-rows-2 p-7 justify-items-center w-screen md:w-3/4 justify-self-center m-auto'>
            ...
            <Image id={idsUrls[0].id} width={200} height={300} onClick={() => {openModal(idsUrls[0].id)}} className='h-auto w-full md:w-auto md:h-64 rounded-md md:hover:rotate-[-10deg] duration-500 cursor-pointer' src={'https://drive.google.com/file/d/1TmJDqWgPIqn7tWTDZzYWg7Q25se90nMK/view?usp=sharing'} />
