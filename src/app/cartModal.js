    ...
    const displayItems = () => {
        ...
        for(const id of cart){
            ...
            const randomKey = Math.random() * 12 + Math.random() * 24;
                arr.push(
                <div key={randomKey} className='box-border w-full md:h-[100px] sm:h-fit sm:text-center sm:justify-items-center md:flex sm:grid sm:grid-rows-5 md:flex-row flex-col gap-6 justify-evenly rounded-xl text-white'>
                    ...
                    <button id='delete-button' type='button' className='sm:m-auto justify-self-center self-center w-fit p-1 rounded text-red-600 hover:text-red-800 text-2xl' onClick={()=> {
                        removeFromCart(id)
                    }}>Delete</button>
                    </div>
